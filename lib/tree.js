...
var tree = module.exports = function(ops) {
	...
	self.listeners = {
		...
		greedy: _parent ? _parent.listeners.middleware.concat() : [],
	...
	self.addListener = function(listener, type) {
	...
	self.child = function(path, createIfNotFound) {
		...
		if(createIfNotFound) {
			...
			return _children[name] = tree(name, { param: path.param, 
				parent: self, 
				root: _root,
				depth: ops.depth + 1, 
				deepest: 0 });
	...
	self.findChild = function(paths, index, weighTowardsParam) {
		...
		if(childTree) {
			...
			if(!(foundChild = childTree.findListeners(paths, index + 1, false))) {

				//otherwise try finding parameter-based route
				foundChild = childTree.findListeners(paths, index + 1, true);	
