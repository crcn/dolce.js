...
var collection = module.exports = function() {
	...
	self.get = function(channel, ops) {
		...
		var chains = _chains(channel.paths, ops, true).sort(function(a, b) {
			return a[a.length - 1].priority > b[b.length - 1].priority ? -1 : 1;
		});
	...
	self.remove = function(channel, ops) {
		...
		for(var i = child.collections.after.length; i--;) {
			if(sifter.test(child.collections.after[i])) {
				child.collections.after.splice(i, 1);
			}
		}
