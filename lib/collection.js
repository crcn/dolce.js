...
var collection = module.exports = function() {
	...
	self.contains = function(typeOrPaths, ops) {

		if(!ops) ops = {};

		var child = _findTree(typeOrPaths instanceof Array ? typeOrPaths : _parseChannel(type).paths);
	...
	self.get = function(type, ops) {
		...
		chains = _chains(channel.paths, tags, true);
	...
	var _chains = function(paths, tags) {
		...
		entireChain = _allCollections(child, usedGreedyPaths),
		...
		for(var i = 0, n = endCollection.length; i < n; i++) {
			...
			//each endpoint may have DIFFERENT metadata, so the chain maybe different
			chain = _chainSifter(currentData.tags, entireChain);


			expandedChains.push(_expand(chain.concat(currentData), paths, _copy(usedGreedyPaths)));
	...
	var _thru = function(paths, tags, usedGreedyPaths) {
		...
		chain = chainSifter(_allCollections(child, usedGreedyPaths));
		...
		return _expand(chain.concat(filteredChildren[0]), paths, usedGreedyPaths);
	}
	...
	var _greedyCollections = function(tree, usedGreedy) {
		...
		while(currentParent) {
			...
			//do NOT re-use greedy middleware. 
			if(usedGreedy[cpath]) break;
	...
	var _allCollections = function(tree, usedGreedy) {
		
		return _greedyCollections(tree, usedGreedy).concat(tree.collections.before);
