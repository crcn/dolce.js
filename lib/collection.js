...
var collection = module.exports = function() {
	...
	var _collections = function(paths, tags) {
		...
		for(var i = 0, n = endCollection.length; i < n; i++) {
			...
			//each endpoint may have DIFFERENT metadata, so the middleware maybe different
			middleware = _middlewareSifter(currentData.tags, allMiddleware);
	...
	var _middlewareSifter = function(tags, target) {
		...
		var test = function(target) {
			...
			return target.filter(function(a) {
				...
				//metadata in the atags (middleware) must match the tags given
	...
	var _thru = function(paths, tags) {
		...
		var middlewareSifter = _middlewareSifter(tags);

		middleware = middlewareSifter(_allCollections(child));

		//need to sort the tags because a match for say.. method=DELETE matches both method, and method=DELETE
		var filteredChildren = middlewareSifter(child.collections.after).sort(function(a, b) {
