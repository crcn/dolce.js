...
var collection = module.exports = function() {
	...
	var _chains = function(segments, ops) {
		...
		if(!child) return [];//_greedyEndpoint(segments, tags);
		...
		for(var i = 0, n = endCollection.length; i < n; i++) {
			...
			expandedChains.push((ops.expand == undefined || ops.expand == true) ? _chain(currentData, segments, entireChain) : [currentData]);
		}
	...
	var _chain = function(data, segments, entireChain) {
		...
		return _expand(chain.concat(data), segments).filter(function(route) {
	...
	var _expand = function(chain, segments) {
	...
	var _thru = function(segments, tags) {
		...
		return _expand(chain.concat(targetChild), segments);
	}
